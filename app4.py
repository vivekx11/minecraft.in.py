...
BLOCK_TYPES = {
    'grass': {'color': color.rgb(50, 205, 50), 'texture': 'white_cube'},
    'stone': {'color': color.rgb(169, 169, 169), 'texture': 'white_cube'},
    'wood': {'color': color.rgb(139, 69, 19), 'texture': 'white_cube'},
    'sand': {'color': color.rgb(255, 218, 185), 'texture': 'white_cube'},
    'gold': {'color': color.rgb(255, 215, 0), 'texture': 'white_cube'}
}
...
# Optimized terrain generation - smaller and colorful
def generate_terrain():
    size = game_state['render_distance']
    for z in range(size):
        for x in range(size):
            # Simpler, flatter terrain for better performance
            height = int(abs(sin(x * 0.5) * cos(z * 0.5)))
            height = max(0, min(height, 1))
            
            # Colorful ground layer with variation
            if (x + z) % 4 == 0:
                Voxel(position=(x, 0, z), block_type='sand')
            elif (x + z) % 3 == 0:
                Voxel(position=(x, 0, z), block_type='stone')
            else:
...
sun.color = color.rgb(255, 250, 200)  # Warm sunlight
AmbientLight(color=color.rgba(150, 180, 255, 0.4))  # Blue ambient light
...
# Sky with vibrant gradient
Sky(color=color.rgb(100, 150, 255))  # Bright blue sky
